...
# Core image/video generation node names to remove in DiffyUI-only mode.
# Keeps: SaveImage, PreviewImage, LoadImage, LoadImageOutput, ImageBatch (used by NIfTI preview / Brain 3D Viewer).
DIFFYUI_CORE_NODES_TO_REMOVE = {
    # Samplers
    "KSampler", "KSamplerAdvanced",
    # Model / checkpoint loaders
    "CheckpointLoaderSimple", "CheckpointLoader", "VAELoader", "LoraLoader", "CLIPLoader", "UNETLoader", "DualCLIPLoader",
    "ControlNetLoader", "DiffControlNetLoader", "StyleModelLoader", "CLIPVisionLoader", "DiffusersLoader",
    "unCLIPCheckpointLoader", "GLIGENLoader", "GLIGENTextBoxApply",
    # Conditioning
    "CLIPTextEncode", "CLIPSetLastLayer", "CLIPVisionEncode", "StyleModelApply", "unCLIPConditioning",
    "ControlNetApply", "ControlNetApplyAdvanced", "InpaintModelConditioning",
    ...
    "ConditioningZeroOut", "ConditioningSetTimestepRange", "LoraLoaderModelOnly",
    # VAE / latent
    "VAEDecode", "VAEEncode", "VAEEncodeForInpaint", "VAEDecodeTiled", "VAEEncodeTiled",
    "EmptyLatentImage", "LatentUpscale", "LatentUpscaleBy", "LatentFromBatch", "RepeatLatentBatch",
    "SetLatentNoiseMask", "LatentComposite", "LatentBlend", "LatentRotate", "LatentFlip", "LatentCrop",
    "LoadLatent", "SaveLatent",
    # Image manipulation nodes not used in DWI workflows
    "LoadImageMask",      # mask loading from image files (we use FSL BET for masks)
    "ImageScale",         # rescaling images (not needed)
    "ImageScaleBy",       # rescaling images (not needed)
    "ImageInvert",        # inverting pixel values (not needed)
    "ImagePadForOutpaint", # outpainting padding (not needed)
    "EmptyImage",         # blank image creation (not needed)
}


def apply_diffyui_core_filter():
    """Remove core image/video generation nodes when running with --diffyui-only (DiffyUI DWI-only mode)."""
    if not getattr(args, "diffyui_only", False):
        return
    for name in DIFFYUI_CORE_NODES_TO_REMOVE:
        NODE_CLASS_MAPPINGS.pop(name, None)
        NODE_DISPLAY_NAME_MAPPINGS.pop(name, None)
    logging.info("DiffyUI-only mode: removed core image/video generation nodes.")
...
async def init_builtin_extra_nodes():
    ...
    # DiffyUI: load only extras essential for DWI workflows (Preview as Text, string/utils, 3D preview).
    # All image/video generation extras (flux, video, samplers, controlnet, etc.) are omitted.
    extras_files = [
        "nodes_primitive.py",     # INT / FLOAT / STRING / BOOLEAN input primitives
        "nodes_preview_any.py",   # Preview as Text (NIfTI Stats, DeepSeek QC output)
        "nodes_string.py",        # String utilities for workflows
        "nodes_logic.py",         # Generic logic (if/else, compare)
        "nodes_nop.py",           # Note / reroute / passthrough nodes
        "nodes_load_3d.py",       # 3D preview (Brain 3D Viewer mesh output)
    ]
