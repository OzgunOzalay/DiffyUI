...
def get_input_data(inputs, class_def, unique_id, execution_list=None, dynprompt=None, extra_data={}):
    ...
    for x in inputs:
        ...
        # Accept link with slot as int, float, or numeric string (e.g. frontend sends "0")
        link_ok = is_link(input_data)
        if not link_ok and isinstance(input_data, list) and len(input_data) == 2 and isinstance(input_data[0], str):
            try:
                _slot = int(input_data[1]) if isinstance(input_data[1], str) else input_data[1]
                link_ok = isinstance(_slot, (int, float))
            except (TypeError, ValueError):
                pass
        if link_ok and (not input_info or not input_info.get("rawLink", False)):
            ...
            # merge_result_data stores each output as a list (batch dim); unwrap single element so downstream gets scalar
            if isinstance(obj, list) and len(obj) == 1:
                obj = obj[0]
            # Wrap in list so slice_dict() passes the full value (batch size 1); unwrapped strings would be indexed by character
        ...
        if isinstance(val, list):
            ...
            if isinstance(slot_index, str):
                try:
                    ...
                    errors.append({"type": "bad_linked_input", "message": "Bad linked input", "details": f"{x}", "extra_info": {"input_name": x, "received_value": val}})
                    continue
            o_class_type = prompt[o_id]['class_type']
            r = nodes.NODE_CLASS_MAPPINGS[o_class_type].RETURN_TYPES
            if slot_index < 0 or slot_index >= len(r):
                errors.append({"type": "bad_linked_input", "message": "Bad linked input, slot index out of range", "details": f"{x}", "extra_info": {"input_name": x, "received_value": val}})
                continue
