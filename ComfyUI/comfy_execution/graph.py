    ...
    def add_node(self, node_unique_id, include_lazy=False, subgraph_nodes=None):
        ...
        while len(node_ids) > 0:
            ...
            for input_name in inputs:
                ...
                # Accept link as [node_id, slot] with slot as int, float, or numeric string (frontend may send "0")
                from_node_id, from_socket = None, None
                if is_link(value):
                    from_node_id, from_socket = value[0], value[1]
                elif isinstance(value, list) and len(value) == 2 and isinstance(value[0], str):
                    try:
                        slot = int(value[1]) if isinstance(value[1], str) else value[1]
                        if isinstance(slot, (int, float)):
                            from_node_id, from_socket = value[0], slot
